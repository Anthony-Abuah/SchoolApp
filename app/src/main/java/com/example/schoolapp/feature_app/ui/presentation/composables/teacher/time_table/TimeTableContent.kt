...
fun TimeTableContent(
    ...
    Column(modifier = Modifier
        ...
        ClassTermHeader(className = className, termName = termName)

        DaysOfTheWeekRow(days = DaysOfTheWeek.values().map { it.fullName })

        BasicScreenColumn {
            teachers.forEach { teacherInfo ->
                TimeTableCard(subjectName = teacherInfo.name,
                    startTime = "8:00 AM",
                    endTime = "10:00 AM"
                ) {
